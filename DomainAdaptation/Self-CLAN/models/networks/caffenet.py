...
class AlexNetCaffeFC7(AlexNetCaffe):
    def __init__(self, aux_classes=1000, n_classes=100, dropout=True):
        ...
        self.aux_classifier = nn.Sequential(OrderedDict([
            ...
            ("relu7", nn.ReLU(inplace=True)),
            ("drop7", nn.Dropout()),
            ("fc8", nn.Linear(4096, aux_classes))]))
...
def caffenet(aux_classes, classes):
    model = AlexNetCaffe(aux_classes, classes)
...
def caffenet_gap(aux_classes, classes):
    model = AlexNetCaffe(aux_classes, classes)
    state_dict = torch.load(os.path.join(os.path.dirname(__file__), "pretrained/alexnet_caffe.pth.tar"))
...
def caffenet_fc7(aux_classes, classes):
    model = AlexNetCaffeFC7(aux_classes, classes)
    state_dict = torch.load("models/pretrained/alexnet_caffe.pth.tar")
    state_dict["aux_classifier.fc7.weight"] = state_dict["classifier.fc7.weight"]
    state_dict["aux_classifier.fc7.bias"] = state_dict["classifier.fc7.bias"]
    ...
    del state_dict["classifier.fc7.bias"]
    model.load_state_dict(state_dict, strict=False)
    nn.init.xavier_uniform_(model.aux_classifier.fc8.weight, .1)
    nn.init.constant_(model.aux_classifier.fc8.bias, 0.)
