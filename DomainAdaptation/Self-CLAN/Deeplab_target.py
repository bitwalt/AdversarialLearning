...
class Self_CLAN:
    def __init__(self, args, logger):
        ...
        self.losses = {'seg': list(), 'seg_t': list(), 'adv': list(), 'weight': list(), 'ds': list(), 'dt': list(),
                       'aux': list()}
        ...
        if args.method.adversarial:
            self.model_D = Discriminator(type_d=args.discriminator.type, num_classes=args.num_classes,
                                         restore=args.restore, restore_from=args.discriminator.restore_from)
            self.optimizer_D = optim.Adam(self.model_D.parameters(), lr=args.discriminator.optimizer.lr,
                                          betas=(0.9, 0.99))
        if args.method.self:
            self.model_A = get_auxiliary_net(args.auxiliary.name)(input_dim=args.auxiliary.classes,
                                                                  aux_classes=args.auxiliary.aux_classes,
                                                                  classes=args.auxiliary.classes)
    ...
    def train(self, src_loader, tar_loader, val_loader):
        ...
        loss_seg = loss_rot = loss_adv = loss_weight = loss_D_s = loss_D_t = 0
        args = self.args
        log = self.logger
        device = self.device

        interp_source = nn.Upsample(size=(args.datasets.source.images_size[1], args.datasets.source.images_size[0]),
                                    mode='bilinear', align_corners=True)
        interp_target = nn.Upsample(size=(args.datasets.target.images_size[1], args.datasets.target.images_size[0]),
                                    mode='bilinear', align_corners=True)
        interp_prediction = nn.Upsample(size=(args.auxiliary.images_size[1], args.auxiliary.images_size[0]),
                                        mode='bilinear', align_corners=True)
        ...
        for i_iter in range(self.start_iter, self.num_steps):
            ...
            if args.method.adversarial:
                ...
                adjust_learning_rate(self.optimizer_D, self.preheat, args.num_steps, args.power, i_iter,
            ...
            if args.method.self:
                ...
                adjust_learning_rate(self.optimizer_A, self.preheat, args.num_steps, args.power, i_iter,
            ...
            #
            # # Segmentation Loss
            # loss_seg = (loss_calc(self.num_classes, pred_source1, labels_s, device) + loss_calc(self.num_classes,
            #                                                                                     pred_source2, labels_s,
            #                                                                                     device))
            # loss_seg.backward()
            ...
            if args.method.adversarial:
                ...
                if i_iter > self.preheat:
                    loss_adv = self.weighted_bce_loss(D_out,
                                                      torch.FloatTensor(D_out.data.size()).fill_(self.source_label).to(
                                                          device), weight_map, args.Epsilon, args.Lambda_local)
                else:
                    loss_adv = self.bce_loss(D_out,
                                             torch.FloatTensor(D_out.data.size()).fill_(self.source_label).to(device))
            ...
            if args.weight_loss:
                ...
                # Initialiaze container variables of DeepLab weights of layers 5 and 6
                W5 = None
                W6 = None
                # TODO: ADD ERF-NET
            ...
            if args.method.adversarial:
                ...
                loss_D_s = self.bce_loss(D_out_s,
                                         torch.FloatTensor(D_out_s.data.size()).fill_(self.source_label).to(device))
                loss_D_s.backward()
                ...
                # Adaptive Adversarial Loss
                if i_iter > self.preheat:
                    loss_D_t = self.weighted_bce_loss(D_out_t, torch.FloatTensor(D_out_t.data.size()).fill_(
                        self.target_label).to(device), weight_map, args.Epsilon, args.Lambda_local)
                else:
                    loss_D_t = self.bce_loss(D_out_t,
                                             torch.FloatTensor(D_out_t.data.size()).fill_(self.target_label).to(device))
            ...
            if i_iter % 10 == 0:
                log.info(
                    'Iter = {0:6d}/{1:6d}, loss_seg = {2:.4f} loss_rot = {3:.4f}, loss_adv = {4:.4f}, loss_weight = {5:.4f}, loss_D_s = {6:.4f} loss_D_t = {7:.4f}'.format(
                        i_iter, self.num_steps, loss_seg_t, loss_rot, loss_adv, loss_weight, loss_D_s, loss_D_t))
